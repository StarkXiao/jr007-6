...
- **确认（新鲜度）**：其他用户在条目详情页点"我确认信息仍然准确"，或"信息已过期"。
  ...
  - 过期上报达到 3 次 → 条目进入 `stale` 标记，地图上显示问号徽标，并生成一个 `stale_recheck`（新鲜度复核）任务投放到审核队列；
  - 同一用户对同一条目 30 天内只能确认一次（服务端事务 + 行锁保证并发下不会写入重复确认）；
  - **众包自愈**：已打标的条目在 90 天内累计拿到 2 次"仍然准确"确认、且新鲜度回到 30 分以上时，自动解除 `stale` 标记并关闭待复核任务，不必等审核员介入；
  - **人工复查**：审核员对 `stale_recheck` 任务"复查通过"即视为权威确认，解除降权、清空历史过期上报数、补一条审核员确认，时间衰减从复查当日重新起算；驳回或要求修改则按普通审核流程处理。

**闭环出口**：评论有举报通道；被隐藏的评论有申诉通道；`stale` 条目有复核任务，经众包再确认或人工复查后回到正常，或驳回下架。
...
4. `freshness_score` 由定时任务重算，规则：`基础分 50 + 确认数×10 − 天数衰减(每 30 天 −10) − 过期上报数×15`，钳制在 0–100。衰减锚点为最近一次准确确认（无确认时取发布时间）。
5. `stale` 自动打标口径（满足任一）：过期上报 ≥ 3 次；距上次确认超过 180 天；无任何确认且分数跌破 30。同一 spot 同时只允许存在一个未决复核任务（`review_tasks.decided_at IS NULL` 去重）。
