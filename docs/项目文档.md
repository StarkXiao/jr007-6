...
- **确认（新鲜度）**：其他用户在条目详情页点"我确认信息仍然准确"，或"信息已过期"。
  ...
  - 过期上报达到 3 次 → 条目进入 `stale` 标记，地图上显示问号徽标，并生成一个 `kind=stale_recheck` 的"待复核"任务投放到审核队列（同一任务不重复入队）；
  - 每日 `stale_sweep` 按时间衰减重算：发布约 90 天无人确认且分数跌破 30 的条目同样自动降权打标、回灌队列；
  - **闭环出口**：人工复查确认仍然有效 → 摘牌、旧账清零；确认失效 → 下架（不扣信用分）、可申诉；
    若近期有 ≥ 3 位用户实地确认且分数回到 60 以上，标记也会被众包自动摘除，不必等人工；
  - 同一用户对同一条目 30 天内只能确认一次；不能确认自己提交的条目。
...
CREATE TABLE spots (
  ...
  freshness_reset_at TIMESTAMPTZ,
...
CREATE TABLE review_tasks (
  ...
  kind             review_task_kind NOT NULL DEFAULT 'submission',
...
4. `freshness_score` 由确认行为实时重算并由每日 `stale_sweep` 兜底重算，规则：`基础分 50 + 本轮确认数×10 − 天数衰减(自上次确认起每 30 天 −10) − 本轮过期上报数×15`，钳制在 0–100。
5. **新鲜度闭环（打标必须能摘牌）**：
   - 降权打标：过期上报 ≥ 3 次，或分数 < 30 且本轮零确认（约等于发布后 90 天无人确认）→ `is_stale = true`，地图显示问号徽标，并回灌一条 `kind = stale_recheck` 的人工复查任务（同一条目未决复查任务只保留一条）。
   - 人工复查出口：复查确认仍然有效 → 摘牌、写入一条权威人工确认、`freshness_reset_at` 置为当前（旧账清零）；确认失效 → 条目转 `hidden` 下架（不扣作者信用分），作者 7 天内可申诉。
   - 众包翻案：已打标条目在 90 天窗口内获得 ≥ 3 条准确确认且分数回到 ≥ 60 → 自动摘牌并重置统计基准，无需等待人工。
   - `confirm_count / stale_report_count` 只统计 `freshness_reset_at` 之后的记录，避免一次过期被永久记账。
...
```json
{
  ...
  "freshness": { "score": 70, "confirmCount": 3, "staleReportCount": 0, "isStale": false, "lastConfirmedAt": "2026-09-01T10:00:00Z" },
...
**POST /spots/:uuid/confirm**

请求体 `{ "isAccurate": true | false, "note"?: string }`。返回：

```json
{ "confirmCount": 3, "staleReportCount": 0, "freshnessScore": 80, "isStale": false, "recovered": false, "addedReviewTask": false }
```

- `isAccurate=false` 使本轮过期上报达阈值时 `addedReviewTask=true`，回灌 `stale_recheck` 任务；
- 已打标条目因近期多人确认翻案时 `recovered=true`、`isStale` 翻回 false；
- 30 天内重复确认返回 409 `ALREADY_CONFIRMED`；确认自己的条目返回 400。
...
| 方法 | 路径 | 权限 | 说明 |
|---|---|---|---|
| GET | `/moderation/queue` | moderator | 待审队列（筛选/排序/分页，`kind=submission|stale_recheck` 区分新提交与过期复查） |
| POST | `/moderation/tasks/:id/claim` | moderator | 领取任务（加锁 30 分钟） |
| POST | `/moderation/tasks/:id/release` | moderator | 释放任务 |
| GET | `/moderation/tasks/:id` | moderator | 任务详情（含 diff、图片、作者历史、众包确认/上报信号） |
| POST | `/moderation/tasks/:id/approve` | moderator | 新提交→通过发布；过期复查→确认仍有效，摘牌并清零旧账 |
| POST | `/moderation/tasks/:id/request-changes` | moderator | 要求修改（需 reason code；过期复查任务不适用） |
| POST | `/moderation/tasks/:id/reject` | moderator | 新提交→驳回；过期复查→确认失效下架（hidden，不扣信用分，可申诉） |
